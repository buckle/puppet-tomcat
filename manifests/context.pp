...
== Definition: tomcat::context

This definition will create a context in a dedicated file
...
- *connection_timeout*: (int) the number of milliseconds this context
...
- *manage*: only add this file/context if it isn’t already present
...
define tomcat::context(
  ...
  file {"${tomcat::params::instance_basedir}/${instance}/conf/context-${name}.xml":
    ensure  => $ensure,
    owner   => $owner,
    group   => $group,
    mode    => $filemode,
